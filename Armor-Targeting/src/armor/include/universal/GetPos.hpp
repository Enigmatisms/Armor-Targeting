...
struct Target{
	...
	Target(){}
    Target(cv::Point2f _pts[4], int _l1, int _l2, int _pNum):
...
public:
	GetPos();
	~GetPos();
	void getDistance(Target tar);			//solvePNP所在的函数	
...
private:
	std::vector<cv::Point2f> sortPoints(Target tar); 
	std::vector<cv::Point2f> sortPoints(cv::RotatedRect rect); 	//重载
private:
...
GetPos::GetPos() {
	...
	//pitches.resize(4);
	//yaws.resize(4);
	g_ctrl.Init(0, 7.95, -4.56, 0.0, 0.0, 12.0, 0.000814);
}
...
void GetPos::getDistance(Target tar) {
	...
	g_ctrl.Transform(cv::Point3f(tVec.at<double>(0, 0),
		tVec.at<double>(1, 0), tVec.at<double>(2, 0)), now_pitch, now_yaw);	//弹道模型云台位置解算
...
void GetPos::getDistance(cv::RotatedRect rect) {
	...
	cv::solvePnP(cv::InputArray(objPoints), cv::InputArray(rect), 
		cv::InputArray(insM), cv::InputArray(distCoeffs), 
		cv::OutputArray(rVec), cv::OutputArray(tVec),
		false, cv::SOLVEPNP_ITERATIVE);
	//std::cout<<tVec<<std::endl;
